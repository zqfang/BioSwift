...
public class VCF2 {
    ...
    public func read2write(filename: String ) {
        let data = try! Data(contentsOf: self.input)
        let vcf = data.withUnsafeBytes {
                return $0.split(separator: UInt8(ascii: "\n"))
                .map{ String(decoding: UnsafeRawBufferPointer(rebasing: $0), as: UTF8.self) }
        }

        let vcff = vcf.map{ll in return self.parse(ll)}
        self.outlines = vcff.filter{ $0 != nil } as! [String]
...
extension VCF2 {
    ...
    func parse(_ line1: String) -> String?
    {
        if line1.starts(with: "##") {
            return nil
        }
        let line = line1.trimmingCharacters(in: CharacterSet.whitespacesAndNewlines)
        // wrirte header
        if line.starts(with: "#CHROM") {
            // good use of components separtedBy
            var st0 = "C57BL/6J" + line.components(separatedBy: "FORMAT")[1] + "\n"
